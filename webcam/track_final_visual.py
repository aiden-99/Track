...
def main():
    ...
    blue_pub = rospy.Publisher('left_rubber', Float64MultiArray, queue_size=1)
    yellow_pub = rospy.Publisher('right_rubber', Float64MultiArray, queue_size=1)
    #Traffic이라는 메시지 타입의 객체를 obj_msg라는 이름으로 생성
    obj_msg = Float64MultiArray()
    blue_msg = Float64MultiArray()
    yellow_msg = Float64MultiArray()
    ...
    while True:
        ...
        blue_msg.data = blue_corn_xyz
        blue_pub.publish(blue_msg)
        yellow_msg.data = yellow_corn_xyz
